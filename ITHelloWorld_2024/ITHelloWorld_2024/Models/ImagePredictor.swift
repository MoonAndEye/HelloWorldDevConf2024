...
class ImagePredictor {
    ...
    func makePredictions(for swiftUIImage: Image, completionHandler: @escaping ImagePredictionHandler) throws {
        /// convert Image to UIImage
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            guard let uiImage = swiftUIImage.render(scale: 1.0) else {
                print("Image can't convert to UIImage")
                return
            }
            do {
                try self.makePredictions(for: uiImage) { predictions in
